<?php
     ...
     * Updated for UK postcode validation by BillJ
     *
...
<script>
    ...
    function check_postcode(field_name, field_size, message) {
        if (form.elements[field_name] && (form.elements[field_name].type != "hidden")) {
            if (field_size == 0) return;
            var field_value = form.elements[field_name].value;
            ...
            if( form.elements['country'].value == 222 ) {
            	...
            	var p = field_value.toUpperCase();
				var parts = p.match(/^([A-Z]{1,2}\d{1,2}[A-Z]?)\s*(\d[A-Z]{2})$/);
				if( parts == null ) {
                	error_message = error_message + "* " + "<?php echo UK_POSTCODE_INCORRECT_FORMAT; ?> " + field_value + "\n";
                	error = true;
            	} else {
					parts.shift();
					p = parts.join(' ');
					
            		// Postcode validation RegEx found here:
            		...
            		// Alternative RegEx from Postcode class
					// var re = new RegExp( '^\s*(([A-Z]{1,2})[0-9][0-9A-Z]?)\s*(([0-9])[A-Z]{2})\s*$' );
            		
            		var re = new RegExp( '^([Gg][Ii][Rr] 0[Aa]{2})|((([A-Za-z][0-9]{1,2})|(([A-Za-z][A-Ha-hJ-Yj-y][0-9]{1,2})|(([AZa-z][0-9][A-Za-z])|([A-Za-z][A-Ha-hJ-Yj-y][0-9]?[A-Za-z])))) [0-9][A-Za-z]{2})$' );
					if( !re.test( p ) ) {
                		error_message = error_message + "* " + "<?php echo UK_POSTCODE_INCORRECT_FORMAT; ?> " + field_value + "\n";
                		error = true;
            		}
            		
            		// If passed verification, update postcode with reformatted value
            		if( !error ) {
            			form.elements[field_name].value = p;
            		}
            	}
            }
        }
    ...
    function check_form(form_name) {
        ...
        check_postcode("postcode", <?php echo (int)ENTRY_POSTCODE_MIN_LENGTH; ?>, "<?php echo ENTRY_POST_CODE_ERROR; ?>");
        <?php } ?>
